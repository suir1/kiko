...
ConnectivityOrchestrator          ← 选路、race、deadline
  ...
  └── rule planner + profile      （规则默认；AI 可选合并）
...
**必须保证 relay 兜底**；在端口映射稳定的环境里，同步 same-port TCP direct 可能成功，但真实 WAN
双 NAT / CGNAT 不能承诺直连。
...
- [x] `ConnectivitySnapshot`、`RoutePlan`、rule planner
...
- [x] U-C-lite：`relay_standby` + direct 可取消，relay 可早提交保底路径
...
- [x] 托管公共 relay + CI 公网冒烟（`KIKO_RELAY_SMOKE=106.53.170.243:9000`）
...
| 双 NAT WAN | relay pipe 100% 完成；端口稳定时 direct 可作为 bonus |
