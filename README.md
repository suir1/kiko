...
`KIKO_INSTALL_DIR` to choose another directory, or `KIKO_VERSION=v0.2.11-alpha` to install a specific release.
...
`kiko tui`. Both Notepad peers must support the same note protocol version; incompatible clients fail with an
upgrade message instead of risking divergent text.
